...
> - **Stealth mode ("Ghost")**: set `stealth_mode = true` in `config.toml` to become indistinguishable from a vanilla console — opponents on any version of the mod see and log nothing. Trade-off: you can't see their extended info either (the mod beacon doubles as the token peers use to decide who to share with). **Lurk mode** (`lurk_mode = true`) is the middle ground: broadcast nothing but keep seeing opponents' info — at the cost of them seeing that you're modded. See the config section below.
>
...
> - New optional `stealth_mode` config key (default `false`). When enabled, the mod never registers the custom-comms send hook, so no packet carrying your latency/render profile is ever broadcast — opponents running any version of the mod receive nothing, see nothing in their overlay, and get nothing in their log, exactly as if you were a vanilla console. Receivers also silently ignore version-0 packets (the zeroed-buffer stealth approach used by v1.2.0-quickplay.2), so stealth players on that build leave no log trace here either.
> - **Manager beacon suppression:** `libssbu_pia_manager.nro` has its own lower-level handshake that rides normal PIA traffic: a `0x45` tag byte plus the console's self-reported `nn::nifm` interface type. The tag powers `is_modded` and the `[Wired]`/`[Wifi]` ping suffix on other consoles — so even with the send hook disabled, the manager itself would still out you as modded. In stealth mode we now patch the tag constant (`mov w8, #0x45` → `mov w8, #0x0`) inside the manager's `.text` at runtime (signature scan bounded by `svcQueryMemory`, patched via `sky_memcpy`), making the console indistinguishable from vanilla at every level we control. If the signature isn't found (unknown manager version), a warning is logged and nothing is patched.
> - **The trade-off (verified against the manager binary):** the `0x45` tag is also the capability token — a manager only answers extended-info requests from stations whose tag it has seen. Suppressing it therefore makes a stealth console blind: opponents won't send it their extended info. True stealth and seeing opponents' info are mutually exclusive with this manager. `lurk_mode` (default `false`) is the opt-in middle ground: send hook stays unregistered (you broadcast nothing) but the beacon flows, so you keep receiving — while opponents can see that you're modded and your connection type.
>
...
- 🥷 **Stealth mode ("Ghost")** *(fork only)*: become indistinguishable from a vanilla console — opponents on any mod version see and log nothing. Trade-off: you can't see their extended info either. Enabled via `stealth_mode = true` in `config.toml` (on by default in the bundled config).
- 👁️ **Lurk mode** *(fork only)*: never broadcast your extended info while still seeing modded opponents' info. Enabled via `lurk_mode = true` in `config.toml`. **Warning**: does *not* hide that you're modded — opponents with the mod see a modded flag and your `[Wired]`/`[Wifi]` connection type. Ignored when stealth mode is on.
...
- **Stealth mode ("Ghost")**: `stealth_mode` controls the "extended opponent info" exchange. When `true`, the mod never broadcasts your latency/render-profile info AND suppresses the lower-level mod-detection beacon inside `libssbu_pia_manager.nro` (the `0x45` tag that powers `is_modded` and the `[Wired]`/`[Wifi]` suffix on opponents' screens), so a stealth console is indistinguishable from a vanilla console at every level we control. **The trade-off**: the `0x45` tag doubles as the capability token — a manager only sends extended info to stations whose tag it has seen — so a stealth console is also **blind**: you will not see opponents' latency/render info while stealth is on. This is structural, not a bug; true stealth and seeing opponents' info are mutually exclusive. The `config.toml` bundled with this fork's releases ships with `stealth_mode = true`. Set it to `false` if you want two-way extended info sharing. (Code default when the key is absent: `false`.)
- **Lurk mode** *(fork only)*: `lurk_mode` is the middle ground. When `true`, your latency/render profile is never broadcast, but the manager beacon is left intact — so you keep seeing modded opponents' extended info. **Warning: lurk mode does not hide the mod.** Opponents running any manager version can see that your console is modded and whether you're on `[Wired]` or `[Wifi]` — they just never see your settings. Ignored when `stealth_mode = true` (stealth is the stronger mode). (Code default: `false`.)
...
stealth_mode = true                         # 'true' = Ghost: vanilla-indistinguishable, but you can't see opponents' info either
lurk_mode = false                           # 'true' = broadcast nothing but still see opponents' info (WARNING: they can see you're modded + your connection type). Ignored if stealth_mode = true
