...
> - **Manager beacon suppression (dev):** `libssbu_pia_manager.nro` has its own lower-level handshake that rides normal PIA traffic: a `0x45` tag byte plus the console's self-reported `nn::nifm` interface type. The tag powers `is_modded` and the `[Wired]`/`[Wifi]` ping suffix on other consoles — so even with the send hook disabled, the manager itself would still out you as modded. In stealth mode we now patch the tag constant (`mov w8, #0x45` → `mov w8, #0x0`) inside the manager's `.text` at runtime (signature scan bounded by `svcQueryMemory`, patched via `sky_memcpy`), making the console indistinguishable from vanilla at every level we control. The receive path is untouched, so you still see everyone else. If the signature isn't found (unknown manager version), a warning is logged and nothing is patched.
>
